...
**Arcade Vault** — a platform to play arcade games online and compete for the high score (see `README.md`). Thirteen games are in the catalog; five have a real engine (`asteroides`, `tetris`, `arkanoid`, `vibora`, `frogger`) and the other eight (`invasores`, `ranaria`, `gloton`, `duelo-pixel`, `bloque-buster`, `caida`, `serpentina`, `rocas`) are still visual mock-ups that fall back to the decorative arena.

The project follows **Spec Driven Design**. Most features landed as a numbered spec in `specs/` (01 → 10, all `Estado: Implementado`), each implemented on its own `spec-NN-slug` branch and merged by PR. Specs 11 → 15 are per-engine performance audits (`Estado: Draft`) written by the `game-performance-booster` agent — measured but not yet built, waiting on a `/spec-impl` pass. `specs/game-jam/<game-id>/` holds a separate, unnumbered spec pair (`spec-diseno.md` + `spec-implementacion.md`) written by the `game-jam` agent for a game before it has a catalog row: `frogger` went from there to a real engine (merged outside the `spec-NN-slug` convention, on branch `juego-frogger`), `invasores` is still `Propuesto`, pending a decision. Specs, code comments and UI copy are written **in Spanish** — match that language when adding to them. Only the top-level agent docs (`CLAUDE.md`, `AGENTS.md`) are in English.
...
- **`skin-designer`** — the one agent here that implements directly instead of only planning. Builds and audits a three-skin system (`clasico` default = today's look untouched, `retro`, `neon`), selectable and persisted per game. Chrome (CRT bezel, HUD, buttons) is CSS custom-property overrides scoped by a `data-skin` attribute on `.av-player` in `components/game-player.tsx`, which covers all thirteen games since mock arenas render there too. The five real engines (`asteroides`, `tetris`, `arkanoid`, `vibora`, `frogger`) additionally get an in-canvas palette: an extension to the engine contract (`GameSkin`, `GameEngine.setSkin` in `lib/games/engine.ts`, mirroring `setMuted`) plus a `skins.ts` sibling file per engine, precedented by `lib/games/tetris/pieces.ts`. Persistence is `lib/skins.ts`, sibling of `lib/preferences.ts`. It has `Edit`/`Write`/`Bash` for exactly this reason; it never touches the catalog row, Supabase, or `specs/`.
...
- **`game-performance-booster`** — audits runtime performance of the five real engines (`asteroides`, `tetris`, `arkanoid`, `vibora`, `frogger`), for per-frame allocations, uncached gradients/`shadowBlur`, O(n²) collisions, and audio-node churn, and measures real FPS by driving each `/jugar/<id>` with Playwright (`run` skill + `requestAnimationFrame` sampling) on top of `lib/games/engine.ts`'s rAF-owned-by-each-engine model. Unlike `skin-designer`/`mobile-porter` it never edits code — it has no `Edit` — and instead writes its findings as a new `specs/NN-slug.md` spec, formatted like `/spec`'s output, with each performance issue as a measurable acceptance criterion for a later `/spec-impl` pass.

## Reference material

`references/` is read-only source material, excluded from lint/format: `templates/` and `home-about/` are the original React UMD mock-ups the UI was ported from, and `started-games/` holds the vanilla-JS originals (`02-asteroids`, `03-tetris`, `04-arkanoid`) that the first three engines were ported from. `vibora` and `frogger` had no originals — both were written from scratch (`frogger` via the `game-jam` agent's spec pair, see Agents above).
